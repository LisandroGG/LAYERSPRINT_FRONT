...
export default function ProductPriceModal({ open, onClose, product }: PriceModalProps) {
	const [tierIndex, setTierIndex] = useState(2)

	if (!open || !product || !product.cost) return null
		...
		<div className="fixed inset-0 z-50 flex items-center justify-center bg-ink/80 p-4">
			<div className="w-full max-w-sm rounded-xl border border-border bg-surface p-6 shadow-xl">
				<div className="flex items-center justify-between">
					...
					<button onClick={onClose} className="cursor-pointer text-muted hover:text-white">✕</button>
				</div>
				<p className="text-sm text-muted">{product.weight}g · {formatMinutesToHours(product.timeToPrint)}</p>

				<div className="mt-4 space-y-2 rounded-lg border border-border bg-ink p-4 font-mono text-sm">
					<div className="flex justify-between text-muted">
						<span>Material</span>
						<span className="text-white">${materialCost.toLocaleString('es-AR')}</span>
					</div>
					<div className="flex justify-between text-muted">
						<span>Electricidad</span>
						<span className="text-white">${energyCost.toLocaleString('es-AR')}</span>
					</div>
					<div className="flex justify-between text-muted">
						<span>Amortización</span>
						<span className="text-white">${machineCost.toLocaleString('es-AR')}</span>
					</div>
					<div className="flex justify-between text-muted">
						<span>Mano de obra</span>
						<span className="text-white">${Number(product.laborCost).toLocaleString('es-AR')}</span>
					</div>
					<div className="flex justify-between text-muted">
						<span>Extras</span>
						<span className="text-white">${Number(product.extras).toLocaleString('es-AR')}</span>
					</div>
				...
				<div className="mt-4">
					...
					<div className="grid grid-cols-3 gap-2">
						{MARGIN_TIERS.map((t, index) => (
							...
							>
								{t.label}
								<span className="block font-mono text-xs opacity-80">x{t.multiplicador}</span>
							</button>
						))}
					</div>
				</div>
